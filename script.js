...
async function weather(city) {
  if (city === "") {
    ...
    ); //declare variable response set to await(make it so it has to grab value of fetch first) fetch the apiUrl with inputed values
    var apiInformation = await response.json(); //declare variable apiInformation set to await for the response.json()

    var responseTwo = await fetch(
      apiUrl2 + "q=" + city + `&units=metric&appid=${apiKey}`
    ); //declare variable responseTwo set to await(make it so it has to grab value of fetch first) fetch the apiUrl with inputed values
    ...
    var imgPrimary = $("<img></img>").attr(
      "src",
      iconUrl + apiInformation.weather[0].icon + ".png"
    ); //declare variable imgPrimary and set equal to element img and set attr src to the image from iconUrl and icon from object of apiInformation
    var pOne = $("<p></p>").text(
      "Temperature: " + Math.round(apiInformation.main.temp) + "°C"
    ); //declare variable pOne and set equal to element p then set text equal to rounded value from apiInformation object
    ...
    var pThree = $("<p></p>").text(
      "Humidity: " + apiInformation.main.humidity + "%"
    ); //declare variable pTwo and set equal to element p then set text equal to  apiInformation object
    ...
    for (i = 0; i < arrayDivs.length; i++) {
      ...
      var img = $("<img></img>").attr(
        "src",
        iconUrl + apiInformationTwo.list[x].weather[0].icon + ".png"
      ); //declare variable img and set equal to element img and set attr src to the image from iconUrl and icon from object of apiInformationTwo
      var pOne = $('<p class="text"></p>').text(
        "Temp: " + Math.round(apiInformationTwo.list[x].main.temp) + "°C"
      ); //declare variable pOne and set equal to element p then set text equal to rounded value from apiInformationTwo object
      var pTwo = $('<p class="text"></p>').text(
        "Wind: " + apiInformationTwo.list[x].wind.speed + "m/s"
      ); //declare variable pTwo and set equal to element p then set text equal to  apiInformationTwo object
      var pThree = $('<p class="text"></p>').text(
        "Humidity: " + apiInformationTwo.list[x].main.humidity + "%"
      ); //declare variable pThree and set equal to element p then set text equal to  apiInformationTwo object
...
function searchHistory(city) {
  ...
  buttons.each(function () {
    if ($(this).text() === city) {
      //if buttons text is equal to the vairble city
  ...
  if (!cityExists) {
    //if it does not equal cityExists
    var button = $('<button class="btn btn-secondary gap-2"></button>').text(
      city
    ); //declare variable button equal to created element of button with classes
